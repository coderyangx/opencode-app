...
import { getLocalIpV4 } from "@mtfe/mns-util";
...
def main():
    ...
    for dataset_id in dataset_ids:
        data_url = f"${host}/analysis-agent/object/{dataset_id}.json"
...
const removeDuplicatedImports = (codeString: string) => {
  ...
  if (allImports.length > 0) {
    ...
    const lastImportIndex =
      codeString.lastIndexOf(lastImportLine) + lastImportLine.length;
...
const getCodeInterceptorTool = async () => {
  ...
  if (process.env.NODE_ENV === "local") {
    transport = new Experimental_StdioMCPTransport({
      ...
      args: [
        "--directory",
        path.join(process.cwd(), "../mcp/python-sandbox"),
        "run",
        "server.py",
      ],
    });
  ...
  const codeTool = Object.keys(tools).find((item) =>
...
const parametersSchema = z.object({
  ...
  analysis_logic: z
    .string()
    .describe("实现该任务所需的 Python 代码核心逻辑的文字描述"),
...
`,
  ...
  const { text } = await generateText({
    model: getLanguageModel("gpt-oss-120b-meituan"),
    temperature: 0.3,
    maxRetries: 1,
    maxSteps: 1,
    messages,
  ...
  return {
    ...
    async execute(args: z.infer<typeof parametersSchema>, options) {
      ...
      const reActLoop = async (roundCtx: ILoopRound) => {
        ...
        try {
          const ret = await nlPythonAnalysis(
            { ...args, datasets },
            ctx,
            roundCtx
          );
          ...
          const finalCode = removeDuplicatedImports(
            CODE_TEMPLATE_FACTORY(
              ...
              process.env.APP_ENV !== "production"
                ? ctx.origin
                : `http://${getLocalIpV4()}:8080`
            )
          );

          const { structuredContent, ...result } =
            await codeInterceptorTool.execute({
              code: finalCode,
              language: "python",
            } as any);
      ...
      while (!roundCxt.forceExit) {
        ...
        roundCxt.forceExit = roundCxt.round >= 4;
        roundCxt.round += 1;
      }
      ...
      if (!finalResult) {
        ...
        return roundCxt.observation
          ? roundCxt.observation &&
            typeof roundCxt.observation.isError !== "undefined"
            ? roundCxt.observation
            : {
                isError: false,
                content: [
