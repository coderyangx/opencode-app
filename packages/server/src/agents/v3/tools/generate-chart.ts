...
export const chartToolFactory: IToolFactory = (c) => {
  return {
    name: "generate-chart",
    description:
      "这是一个强大的数据可视化工具。当你需要将结构化的数据（如Markdown表格中的数据）转化为直观的图表（例如，折线图、柱状图、饼图等）时，请调用此工具。你需要提供明确的图表类型建议、清洗整理后的数据、以及图表标题、X轴、Y轴标签等详细配置信息。请记住，图表是报告中数据洞察的关键组成部分，务必利用此工具增强报告的可读性。",
    parameters: z.object({
      chartOptions: z.object({
        xAxis: z
          ...
          .describe("X 轴配置，当使用 pie 类型图表时，不需要提供该配置")
          .optional(),
        title: z
          .object({
        ...
        series: z
          .array(
            z.object({
              ...
              type: z.string().describe("Chart type (bar, line, pie)"),
              data: z
                .array(
                  z.union([
                    z.number(),
    ...
    async execute(args, options) {
      try {
        ...
        console.log("tool return", Date.now());
